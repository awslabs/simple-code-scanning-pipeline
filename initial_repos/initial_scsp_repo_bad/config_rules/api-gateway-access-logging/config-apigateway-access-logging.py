...
#####################################
Rule Name:
    delta-config-apigateway-access-logging


Description:
  Checks that Amazon API Gateway stages have 'AccessLogSettings' enabled.
  The rule returns NON_COMPLIANT if a stage has 'AccessLogSettings' undefined or incompletely defined.
  The rule returns COMPLIANT if a stage has 'AccessLogSettings' defined.
...
Scenarios:
  Scenario: 1
    Given: In the Stage configuration item, 'AccessLogSetting' and its subkeys 'Format' and 'DestinationArn' are defined.
      Then: Return COMPLIANT
  Scenario: 2
    Given: In the Stage configuration item, 'AccessLogSetting' or 1+ of its subkeys 'Format' and 'DestinationArn' are not defined.
      Then: Return NON_COMPLIANT
  Scenario: 3
    Given: In the Stage configuration item, 'AccessLogSetting.DestinationArn' is defined,
           but does not match the expected destination pattern.
      Then: Return COMPLIANT # TODO - The logic that flags this as non-compliant is currently commented-out,
            but can be enabled when teams are ready to point their destinations to a standard log group.
'''
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    access_log_format = access_log_settings.get("format")
    access_log_destination_arn = access_log_settings.get("destinationArn")
    rest_api_id = stage.get("restApiId")
    stage_name = stage.get("stageName")
    if rest_api_id and stage_name:
        expected_log_group_name = f"/aws/api-gateway/{rest_api_id}/{stage_name}"
    else:
        expected_log_group_name = ""  # if we fail at constructing the expected log group, use a pattern everything will match
    if access_log_settings and access_log_format and access_log_destination_arn:
        if re.search(expected_log_group_name, access_log_destination_arn):
            # Scenario 1: If AccessLogSetting(s) is defined, return compliant.
            return build_evaluation_from_config_item(configuration_item, 'COMPLIANT')
        else:
            return build_evaluation_from_config_item(configuration_item, 'COMPLIANT')
            # Scenario 3: Currently commented out, but can be used to enforce standard log destinations
            # return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', 'Access log destination is not the standard destination.')
    else:
        # Scenario 2: If any AccessLogSetting values are not defined, return non-compliant.
        return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', 'AccessLogSettings are not defined for this stage.')
...
def lambda_handler(event, context):
    ...
    test_mode = False
    if result_token == 'TESTMODE':  # nosec - standard RDK test string
