...
Scenarios:
  Scenario: 1
    Given: In the Stage configuration item, 'AccessLogSetting' and its
           subkeys 'Format' and 'DestinationArn' are defined.
      Then: Return COMPLIANT
  Scenario: 2
    Given: In the Stage configuration item, 'AccessLogSetting' or 1+ of its
        subkeys 'Format' and 'DestinationArn' are not defined.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if rest_api_id and stage_name:
        ...
        # if we fail at constructing the expected log group, use a pattern everything will match
    ...
    if access_log_settings and access_log_format and access_log_destination_arn:
        if re.search(expected_log_group_name, access_log_destination_arn):
            ...
            #     configuration_item,
            #     'NON_COMPLIANT',
            #     'Access log destination is not the standard destination.'
            # )
    else:
        ...
        return build_evaluation_from_config_item(
            configuration_item,
            "NON_COMPLIANT",
            "AccessLogSettings are not defined for this stage.",
        )
